#!/bin/sh
# Aegis install script for Linux and macOS.
#
# Usage (pipe to sh):
...
#   AEGIS_INSTALL_DIR  destination directory   (default: /usr/local/bin,
#                      falling back to $HOME/.local/bin when that is not
#                      writable and no sudo is available)
#   AEGIS_VERIFY_COSIGN   auto|true|false (default auto: verify when cosign
...
# Actions OIDC issuer for the aegis repo.
...
AEGIS_VERIFY_COSIGN="${AEGIS_VERIFY_COSIGN:-auto}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --install-dir)  AEGIS_INSTALL_DIR="$2"; shift 2 ;;
    --no-cosign)    AEGIS_VERIFY_COSIGN=false; shift ;;
    -h|--help)      usage; exit 0 ;;
...
require() {
  command -v "$1" >/dev/null 2>&1 || {
    echo "aegis-install: required tool not found: $1" >&2
    exit 1
  }
}
require curl
...
case "$os" in
  linux|darwin) ;;
  *) echo "aegis-install: unsupported OS: $os" >&2; exit 1 ;;
esac
...
case "$arch" in
  ...
  *) echo "aegis-install: unsupported arch: $arch" >&2; exit 1 ;;
esac
...
if [ "$AEGIS_VERSION" = "latest" ]; then
  ...
  base="https://github.com/${REPO}/releases/download/${AEGIS_VERSION}"
fi

tmp=$(mktemp -d 2>/dev/null || mktemp -d -t aegis-install)
cleanup() { rm -rf "$tmp"; }
...
download "${base}/${asset}"         "${tmp}/aegis"
download "${base}/${asset}.sha256"  "${tmp}/aegis.sha256"

# Pick whichever sha256 tool is available.
if command -v sha256sum >/dev/null 2>&1; then
  actual=$(sha256sum "${tmp}/aegis" | awk '{print $1}')
elif command -v shasum >/dev/null 2>&1; then
  actual=$(shasum -a 256 "${tmp}/aegis" | awk '{print $1}')
else
  echo "aegis-install: no sha256 tool found (install sha256sum or shasum)" >&2
  exit 1
fi
expected=$(awk '{print $1}' "${tmp}/aegis.sha256")

if [ "$expected" != "$actual" ]; then
  echo "aegis-install: SHA256 mismatch — refusing to install" >&2
...
if [ "$AEGIS_VERIFY_COSIGN" = "auto" ]; then
  if command -v cosign >/dev/null 2>&1; then
    AEGIS_VERIFY_COSIGN=true
  else
    AEGIS_VERIFY_COSIGN=false
    echo "• cosign not installed; skipping signature verification (install cosign for stronger guarantees)"
  fi
fi

if [ "$AEGIS_VERIFY_COSIGN" = "true" ]; then
  command -v cosign >/dev/null 2>&1 || {
    echo "aegis-install: --cosign/AEGIS_VERIFY_COSIGN=true but cosign is not on PATH" >&2
    exit 1
  }
  download "${base}/${asset}.sigstore" "${tmp}/aegis.sigstore"
  cosign verify-blob \
    --bundle "${tmp}/aegis.sigstore" \
